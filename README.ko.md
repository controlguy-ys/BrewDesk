...
기존 Homebrew 패키지 관리와 새 Formula·Cask 검색 및 설치를 지원합니다.
...
2. 설치 목록에서 검색·유형 필터를 사용하고 항목을 선택해 상세 정보를 봅니다. 패키지 왼쪽 체크박스로 선택하며 목록 위에 전체 선택·선택 해제·선택 개수를 표시합니다. 전체 선택은 현재 표시된 항목을 추가 선택하고, 선택 해제는 숨겨진 항목까지 모두 해제합니다. 목록에 포커스를 둔 ⌘A는 전체 행을, 검색창에서는 검색어를 선택합니다. 전체 업데이트는 검색·필터와 관계없이 고정되지 않은 모든 업데이트 대상을 확인창에 표시합니다.
...
`--zap`, 강제 제거, root로 Homebrew 실행, 서비스 관리는 제공하지 않습니다. 자체 업데이트·`latest` Cask에는 확인 제한을 표시합니다. 설치 경로는 실제 파일이 확인된 경우만 제공합니다.
...
작업 기록의 성공·실패 제목, 날짜, 오른쪽 빈 공간 등 요약 행 전체를 클릭해 펼치거나 접을 수 있습니다. 펼친 로그의 선택과 복사·저장 버튼은 별도로 동작합니다.

실행 중 `[y/n]`, `(yes/no)`처럼 명시적인 질문이 일반 줄 입력 모드에서 감지되면 예·아니요 팝업을 표시하고 선택한 답을 터미널에 전달합니다. “터미널에서 응답”을 누르면 자동 응답 없이 직접 입력할 수 있습니다. 비밀번호·원시 입력 모드·인식하지 못한 질문은 터미널에서 처리합니다. 인증 후 일반 입력 모드로 돌아오면 팝업 감지를 재개하며, 비밀 입력 이후의 로그 저장 제한은 유지합니다. 자동으로 예를 선택하지 않습니다.
